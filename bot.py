...
# ---------- TOKEN GET KARO (Multiple ways) ----------
BOT_TOKEN = os.getenv("8965822236:AAFZMaabmqiGFKiKSNtZVKpX3RrMX4XM-Uk")  # Render se

# AGAR ENV VARIABLE NAHI MILA TOH DIRECT DALO (TESTING KE LIYE)
if not BOT_TOKEN:
    BOT_TOKEN = "8965822236:AAFZMaabmqiGFKiKSNtZVKpX3RrMX4XM-Uk"  # <-- APNA TOKEN YAHAN DALO (Direct)

# TOKEN CHECK KARO
if not BOT_TOKEN or BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
    print("❌ ERROR: Bot token nahi mila!")
    print("💡 Render pe Environment Variable 'BOT_TOKEN' set karo")
    print("💡 Ya code mein direct token dalo")
    exit(1)

print(f"✅ Token found: {BOT_TOKEN[:10]}... (hidden)")
...
try:
    if GEMINI_API_KEY:
        ...
        print("⚠️ Gemini API key nahi mili, echo mode on")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if AI_AVAILABLE and AI_ENABLED:
        ...
        reply_text = (
            ...
            f"`{user_text}`\n\n"
            f"😊 Bot working! AI key set karein for smart replies."
        )
...
def main():
    print("🚀 Bot starting...")
    print(f"📝 Token: {BOT_TOKEN[:15]}...")  # Token partially show
